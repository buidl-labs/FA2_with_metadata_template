...
class FA2_config:
    def __init__(self,
        ...
        self.add_tickets_transfer = add_tickets_transfer
        # Add an entry point that accepts tickets as transaction orders.
...
## This class provides helpers to create and force the type of such elements.
## It uses the `FA2_config` to decide whether to set the right-comb layouts.
class Batch_transfer_tickets(Batch_transfer):
    def get_transfer_type(self):
        tx_type = self.get_tx_type()
        transfer_type = sp.TTicket(sp.TPair(
                                    sp.TAddress,
                                    sp.TList(tx_type)
                                   ))
        return transfer_type
    def item(self, fa2_dest, txs):
        return sp.ticket(sp.pair(fa2_dest, txs), 1)
##
...
class Token_meta_data:
    ...
    def get_type(self):
        ...
        if self.config.store_total_supply:
            return (sp.TRecord(total_supply = sp.TNat, metadata_map = t))
        else:
            return t
    ...
    def get_metadata(self, expr):
        if self.config.store_total_supply:
            return expr.metadata_map
        else:
            return expr
    def make(self, amount, metadata):
        if self.config.store_total_supply:
            return sp.record(total_supply = amount,
...
## `tickets_transfer` is an optional entry-point, hence we define it “outside” the
## class:
def tickets_transfer(self, params):
    sp.verify( ~self.is_paused() )
    sp.set_type(params, self.batch_transfer_tickets.get_type())
    sp.for transfer in params:
        ticket_data, ticket_copy = sp.read_ticket(transfer)
        current_from, content = sp.match_pair(ticket_data)
        dest_txs, qty = sp.match_pair(content)
        dest, txs = sp.match_pair(dest_txs)
        sp.verify(dest == sp.self_address,
                  message=self.error_message.make("FA2_WRONG_DESTINATION"))
        sp.for tx in txs:
            # sp.verify(tx.amount > 0, message = "TRANSFER_OF_ZERO")
            self.verif_asset_exist(tx.token_id)
            self.do_transfer(tx.token_id, tx.amount * qty, tx.to_, current_from)
##
...
class FA2_core(sp.Contract):
    def __init__(self, config, metadata_uri, **extra_storage):
        self.config = config
        ...
        self.batch_transfer_tickets = Batch_transfer_tickets(self.config)
        if  self.config.add_mutez_transfer:
            self.transfer_mutez = sp.entry_point(mutez_transfer)
        if  self.config.add_tickets_transfer:
            self.tickets_transfer = sp.entry_point(tickets_transfer)
    ...
    def verif_asset_exist(self, token_id):
        if self.config.single_asset:
            sp.verify(token_id == 0, "single-asset: token-id <> 0")
        sp.verify(self.data.tokens.contains(token_id),
                          message = self.error_message.token_undefined())

    def verif_transfer_rights(self, tx, current_from):
    ...
    def do_transfer(self, token_id, amount, to_, current_from):
        # If amount is 0 we do nothing now:
        sp.if (amount > 0):
            from_user = self.ledger_key.make(current_from, token_id)
            sp.verify(
                (self.data.ledger[from_user].balance >= amount),
                message = self.error_message.insufficient_balance())
            to_user = self.ledger_key.make(to_, token_id)
            self.data.ledger[from_user].balance = sp.as_nat(
                self.data.ledger[from_user].balance - amount)
            sp.if self.data.ledger.contains(to_user):
                self.data.ledger[to_user].balance += amount
            sp.else:
                 self.data.ledger[to_user] = Ledger_value.make(amount)
...
## This contract is used by the tests to generate pay tickets
class Ticket_payer(sp.Contract):
    def __init__(self, config):
        self.batch_transfer_tickets = Batch_transfer_tickets(config)
        self.init()

    @sp.entry_point
    def pay(self, params):
        content = sp.record(to_=params.to_, token_id=params.token_id, amount=params.amount)
        ticket = self.batch_transfer_tickets.item(params.fa2_dest, [content])
        t = self.batch_transfer_tickets.get_type()
        # dest = sp.contract(t, params.fa2_dest, "transfer_tickets")
        dest = sp.contract(t, params.fa2_dest, "tickets_transfer")
        sp.transfer(sp.list([ticket]), sp.mutez(0), dest.open_some())
...
def add_test(config, is_default = True):
    ...
    def test():
        ...
        c1 = FA2(config,
                 metadata = sp.bytes_of_string("https://example.com"),
                 admin = admin.address)
        scenario += c1
        ...
        scenario.verify(
            ...
            == 10 + 10 + 11)
        if config.add_tickets_transfer:
            scenario.h2("A new test contract ticket_payer is created")
            ticket_payer = Ticket_payer(config)
            scenario += ticket_payer
            scenario.h2("Alice -> ticket_payer")
            scenario += c1.transfer(
            [
                c1.batch_transfer.item(from_ = alice.address,
                                    txs = [
                                        sp.record(to_ = ticket_payer.address,
                                                  amount = 10,
                                                  token_id = 0)
                                    ])
            ]).run(sender = alice)
            scenario.h2("tickes_payer -> Bob via Transfer Tickets")
            scenario += ticket_payer.pay(to_ = bob.address,
                                         amount = 10,
                                         token_id = 0,
                                         fa2_dest = c1.address)
            scenario.verify(
            c1.data.ledger[c1.ledger_key.make(ticket_payer.address, 0)].balance
                == 10 - 10)
            scenario.verify(
            c1.data.ledger[c1.ledger_key.make(bob.address, 0)].balance
                == 10 + 10 + 11 + 10)
            scenario.h2("Bob -> Alice")
            scenario += c1.transfer(
            [
...
def environment_config():
    return FA2_config(
        ...
        add_tickets_transfer = global_parameter("add_tickets_transfer", False),
...
if "templates" not in __name__:
    ...
    if not global_parameter("only_environment_test", False):
        ...
        add_test(FA2_config(non_fungible = True, add_tickets_transfer = True),
                 is_default = not sp.in_browser)
        ...
        add_test(FA2_config(add_mutez_transfer = True)
                 , is_default = not sp.in_browser)
        add_test(FA2_config(add_tickets_transfer = True)
                 , is_default = not sp.in_browser)
