...
class Token_meta_data:
    ...
    def get_type(self):
        ...
        if self.config.store_total_supply:
            return (sp.TRecord(token_info = t, total_supply = sp.TNat))
        else:
            return (sp.TRecord(token_info = t))
    ...
    def make(self, amount, metadata):
        if self.config.store_total_supply:
            return sp.record(token_info = metadata, total_supply = amount)
        else:
            return sp.record(token_info = metadata)
...
class FA2_token_metadata(FA2_core):
    ...
    def token_metadata(self, tok):
        ...
        sp.result(
            sp.pair(tok + 0,
                    self.token_meta_data.get_metadata(self.data.token_metadata[tok]))
        )
...
class FA2(FA2_change_metadata, FA2_token_metadata, FA2_mint, FA2_administrator, FA2_pause, FA2_core):
    ...
    def total_supply(self, tok):
        if self.config.store_total_supply:
            sp.result(self.data.token_metadata[tok].total_supply)
        else:
            sp.set_type(tok, sp.TNat)
...
def add_test(config, is_default = True):
    ...
    def test():
        ...
        tok0_md = {"name" : sp.bytes_of_string("The Token Zero"),}
        ...
        tok1_md = {"name" : sp.bytes_of_string("The Token Zero"),}
        scenario += c1.mint(address = bob.address,
                            amount = 100,
                            metadata = tok1_md,
                            token_id = 1).run(sender = admin)
        tok2_md = {"name" : sp.bytes_of_string("The Token Zero"),}
        scenario += c1.mint(address = bob.address,
                            amount = 200,
                            metadata = tok2_md,
